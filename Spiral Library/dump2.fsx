...
let gemm2 transa transb (alpha: floatType) (A:dMatrix) (B:dMatrix) beta (C:dMatrix) =
    ...
    if m <> C.num_rows || n <> C.num_cols then 
        printfn "%A" A
        printfn "%A" B
        printfn "%A" C
        failwith "m <> C.num_rows || n <> C.num_cols in gemm2"
...
/// Can be used for both matrix-matrix standard and Hadamarad multiplications, but it is not intended that they be used at the same time.
/// It might be good to split this function for that reason.
let linear_layer (mm: (DM*DM ref) []) (hads: (DM*DM ref) []) (bias: DM option) =
    let node = DM_rec.createEmpty
    let c = node.P
    let error = node.A
    ...
    let ff() =
        if mm.Length > 0 then 
            ...
            let nc = (!r.contents.r.P).num_cols
            node.Resize nr nc
        ...
        for l,r in mm do gemm2 nT nT 1.0f !l.r.P !r.contents.r.P 1.0f !c
        for l,r in hads do hadamaradMultiplicationErrorModule.A(!l.r.P, !r.contents.r.P, !c, !c)

    let fb() =
        for l,r in mm do
            if l.r.is_constant = false then gemm2 nT T 1.0f !error !r.contents.r.P 1.0f !l.r.A
            if r.contents.r.is_constant = false then gemm2 T nT 1.0f !l.r.P !error 1.0f !r.contents.r.A
        for l,r in hads do 
            hadamaradMultiplicationErrorModule.A(!error, !r.contents.r.P, !l.r.A, !l.r.A)
            hadamaradMultiplicationErrorModule.A(!l.r.P, !error, !r.contents.r.A, !r.contents.r.A)
...
let cross_entropy_cost target activations =
    let cross_ent = linear_layer [||] [|target,ref <| log_ activations;scalar_matrix_add 1.0f -1.0f target,ref <| log_ (scalar_matrix_add 1.0f -1.0f activations)|] None
    let s = sum cross_ent
...
type GRULayer =
    ...
    member l.runLayerNoI (y) =
        ...
        let potential_new_state = linear_layer [|l.U_n, ref <| linear_layer [||] [|reset_gate, y|] None|] [||] (Some l.b_n) |> l.a
        linear_layer [||] [|update_gate,y;(scalar_matrix_add 1.0f -1.0f update_gate),ref potential_new_state|] None
    ...
    member l.runLayer (x) (y) =
        ...
        let potential_new_state = linear_layer [|l.W_n,x;l.U_n, ref <| linear_layer [||] [|reset_gate, y|] None|] [||] (Some l.b_n) |> l.a
        linear_layer [||] [|update_gate,y;(scalar_matrix_add 1.0f -1.0f update_gate),ref potential_new_state|] None
...
type LSTMLayer =
    ...
    member l.runLayer (x) (y) (c) =
        ...
        let c' = linear_layer [||] [|block_input,ref input_gate;!c,ref forget_gate|] None
        let output_gate = linear_layer [|l.W_o,x;l.U_o,y;l.P_o,ref c'|] [||] (Some l.b_o) |> sigmoid
    ...
    member l.runLayerNoH (x) =
        ...
        let output_gate = linear_layer [|l.W_o,x;l.P_o,ref c'|] [||] (Some l.b_o) |> sigmoid
        hadmult (l.block_output_a c') output_gate, c'

    member l.runLayerNoI (y) (c) =
        ...
        let c' = linear_layer [||] [|block_input,ref input_gate;!c,ref forget_gate|] None
        let output_gate = linear_layer [|l.U_o,y;l.P_o,ref c'|] [||] (Some l.b_o) |> sigmoid
        hadmult (l.block_output_a c') output_gate, c'
